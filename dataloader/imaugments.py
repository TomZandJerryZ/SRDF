...
from dataloader.bbox import quad_2_rbox, rbox_2_quad, mask_valid_boxes
def data_transform(image1, annotation1):
    ...
    num_obj = annotation['boxes'].shape[0]
    annotation['boxes'] = annotation['boxes'].reshape(num_obj,4,2)
    image, annotation['boxes'], crop_center = random_flip(image, annotation['boxes'], crop_center)
    # bbx = annotation['boxes']
    ...
    M = load_affine_matrix(crop_center=crop_center,
                           crop_size=crop_size,
                           dst_size=(DefaultConfig.input_w, DefaultConfig.input_h),
                           inverse=False,
                           rotation=True)
    image = cv2.warpAffine(src=image, M=M, dsize=(DefaultConfig.input_w, DefaultConfig.input_h), flags=cv2.INTER_LINEAR)
    if annotation['boxes'].shape[0]:
        annotation['boxes'] = np.concatenate(
            [annotation['boxes'], np.ones((annotation['boxes'].shape[0], annotation['boxes'].shape[1], 1))], axis=2)
        annotation['boxes'] = np.matmul(annotation['boxes'], np.transpose(M))
        annotation['boxes'] = np.asarray(annotation['boxes'], np.float32)

    out_annotations = {}
    size_thresh = 3
    out_rects = []
    out_cat = []
    for pt_old, cat in zip(annotation['boxes'], annotation['gt_classes']):
        if (pt_old < 0).any() or (pt_old[:, 0] > DefaultConfig.input_w - 1).any() or (pt_old[:, 1] > DefaultConfig.input_h - 1).any():
            pt_new = pt_old.copy()
            pt_new[:, 0] = np.minimum(np.maximum(pt_new[:, 0], 0.),DefaultConfig.input_w - 1)
            pt_new[:, 1] = np.minimum(np.maximum(pt_new[:, 1], 0.),DefaultConfig.input_h - 1)
            iou = ex_box_jaccard(pt_old.copy(), pt_new.copy())
            if iou > 0.6:
                rect = cv2.minAreaRect(pt_new)

                box = cv2.boxPoints(rect)
                # print(box.reshape(8,))
                if rect[1][0] > size_thresh and rect[1][1] > size_thresh:
                    # out_rects.append([rect[0][0], rect[0][1], rect[1][0], rect[1][1], rect[2]])
                    out_rects.append(box.reshape(8, ).tolist())
                    out_cat.append(cat)
        else:
            rect = cv2.minAreaRect(pt_old)
            box = cv2.boxPoints(rect)
            # print(box.reshape(8, ).tolist())
            if rect[1][0] < size_thresh and rect[1][1] < size_thresh:
                continue
            # out_rects.append([rect[0][0], rect[0][1], rect[1][0], rect[1][1], rect[2]])
            out_rects.append(box.reshape(8, ).tolist())
            out_cat.append(cat)
    out_annotations['boxes'] = np.asarray(out_rects, np.float32)
    out_annotations['gt_classes'] = np.asarray(out_cat, np.uint8)
    # out_annotations['old_bbx'] = np.array(bbx)
    # out_annotations['old_img'] = old_img
...
class CopyPaste(object):
    ...
    def __call__(self, img, labels, mode=None):
        ...
        for i, coor in enumerate(object_coors):
            ...
            M_up = np.float32([[1, 0, -h * (1 + self.pos) * np.cos(math.pi * 0.5 + a)],
                               [0, 1, -h * (1 + self.pos) * np.sin(math.pi * 0.5 + a)]])
            M_down = np.float32([[1, 0, h * (1 + self.pos) * np.cos(math.pi * 0.5 + a)],
                                 [0, 1, h * (1 + self.pos) * np.sin(math.pi * 0.5 + a)]])
            # 分别获得bbox上下邻域的梯度
            ...
            if not sobel_up.all() and not sobel_down.all() and random.random() < self.p:
                ...
                pix_diff = abs(
                    (up_masked_img).sum() / (area * 255) - (down_masked_img).sum() / (area * 255))  # 防止模糊图像的梯度都平滑带来误操作
