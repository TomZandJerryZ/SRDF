...
class DOTADataset(data.Dataset):
    ...
    def labels(self):
        ...
        for i in range(0, self.n):
            ...
            root_dir = index.split(r'\\images\\P')[0]
    ...
    def _load_image_names(self):
        ...
        with open(image_set_file) as f:
            ...
            image_list = [os.path.join(DefaultConfig.train_image_p,x.strip()+'.png',) for x in f.readlines()]
...
def rotate_img_bbox(img,bbx,p=0.5):
    ...
    if np.random.rand(1) < p:
        # print(1)
        scale = 1
        random_degree = random.choice([90,180,270,30,60,-30,-60])
...
def labels_to_class_weights(labels, nc=15):
...
def classes_weights():
    ds = DOTADataset(dataset=DefaultConfig.train_txt, augment=True)
    # cw = class_weights.cpu().numpy() * (1 - maps) ** 2 / DefaultConfig.class_num  # class weights
    class_weights = labels_to_class_weights(ds.labels, DefaultConfig.class_num) * DefaultConfig.class_num
...
if __name__ == '__main__':
    ...
    # for i,index in enumerate(ds.classes[1:]):
        # print(i,index)
        # print(i)
        # print(ds.classes[1:][i],class_weights[i])
